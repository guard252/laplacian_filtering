...
namespace retouch
{
                                                          ...
                                                          const double &beta, const double &sigma) const
    {
        ...
        {
            ImageSaver saver;
            saver.savePNG(output_pyramid[i], "../images/output_images/lap" + std::to_string(i) + ".png");
        }
        ...
        for(int layer_number = 0; layer_number < KPyramid_size - 1; layer_number++)
        {
            ...
            for(int y = current_thread; y < KGaussian_height; y += num_of_threads)
            {
                ...
                for(int x = 0; x < KGaussian_width; x++)
                {
                    ...
                    glm::ivec2 subimage_end = {
                            std::min<int>(x_in_full_resolution + KSubimage_radius,
                            gaussian_pyramid[0].getWidth()),
                            y_bottom_bound };
